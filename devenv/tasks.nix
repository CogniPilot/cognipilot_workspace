...
let
  ...
  allTasks = cacheFlakeTasks // {
    "sources:checkout" = {
      description = "Initialize top-level Git submodules at the revisions recorded by the workspace.";
      cwd = root;
      exec = ''
        git submodule sync
    ...
    "sources:status" = {
      description = "Show recorded revisions and worktree status for top-level Git submodules.";
      cwd = root;
      exec = ''
    ...
    "sources:verify" = {
      description = "Verify that every top-level Git submodule is initialized at its recorded revision.";
      cwd = root;
      before = [ "devenv:enterTest" ];
      exec = ''
        git submodule status
        if git submodule status | grep -Eq '^[-+U]'; then
          echo "top-level submodules are missing, mismatched, or conflicted" >&2
          exit 1
        fi
      '';
    };
