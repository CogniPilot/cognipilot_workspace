...
let
  ...
  hasEdge =
    from: to: kind: edges:
    builtins.any (edge: edge.from == from && edge.to == to && edge.kind == kind) edges;
...
assert
  map (launch: launch.coordinate) launchIndex.catalog.launches == [
    "app/router"
    "app/stack"
  ];
assert
  map (instance: instance.instanceId) stack.instances == [
    "app/stack"
    "app/stack/base"
  ];
assert (builtins.elemAt stack.instances 1).parameterBindings.host == "router-host";
assert
  (builtins.elemAt stack.processes 0).dependencies == {
